...
from .variables import V, Variable, Path
...
"""
    ...
    def join(self, *path):
        ...
        self.logger.debug('%s.join(V("basedir"), *%s)', self, path)
        return str(Path(V('basedir'), *path))
...
"""
    ...
    def check_candidate(self, candidate):
        ...
        if not pattern:
            return True
        elif fnmatch.fnmatchcase(str(candidate), pattern):
            return True
        else:
            return False
...
"""
    ...
    def next(self):
        ...
        assert isinstance(mapped, (Path, str)), 'map() must return a str'
